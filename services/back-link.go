...
func (cs *CrawlerService) extractAnchorTags(page_url string, proxyFlag bool, s *models.Spider) (map[string]string, string, int64) {
	...
	tabCtx, tabCancel := chromedp.NewContext(cs.ctx)
	defer tabCancel()
	...
	select {
	case <-cs.ctx.Done():
		fmt.Println("DEBUG: parent ctx is already canceled!")
	default:
		fmt.Println("DEBUG: parent ctx is still alive")
	}
